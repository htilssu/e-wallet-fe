...
const AddInfoAtm = () => {
    ...
    const handleDateChange = (e) => {
        ...
        // Chèn dấu gạch chéo tự động sau khi nhập tháng
        if (value.length === 2 && !value.includes('/')) {
            value = value + '/';
        }
        // Giới hạn độ dài của giá trị nhập vào tối đa 7 ký tự (mm/yyyy)
        if (value.length > 7) {
            value = value.slice(0, 7);
        }

        // Lưu trữ giá trị nhập trước đó
        setPreviousNgayphathanh(ngayphathanh);
        setNgayphathanh(value);

        // Kiểm tra hợp lệ và hiển thị lỗi nếu cần
