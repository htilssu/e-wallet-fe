    ...
    <div className="max-w-2xl bg-white shadow-md rounded-lg overflow-hidden p-3 mb-9">
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-6">
          ...
          />
          {error && (
            <p className="text-red-600 text-sm mt-1">Vui lòng nhập số tiền</p>
          )}
        </div>
        <div className="mb-6">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {paymentMethods.map(({ label, minAmount, fee }) => (
              ...
              >
                <p
                  className={`font-semibold ${method === label ? "text-blue-500" : "text-gray-700"}`}
                >
                  {label}
                </p>
                <p className="text-sm text-gray-500">
                  Tối thiểu: {minAmount.toLocaleString()}đ{" "}
                  {fee && ` - Phí: ${fee}`}
                </p>
              </div>
            ))}
          ...
          {error && !amount && (
            <p className="text-red-600 text-sm mt-1">
              Vui lòng nhập số tiền trước khi chọn phương thức
            </p>
          )}
        </div>
      </form>
      <div className="mt-6 p-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700">
        ...
        <p>
          Đối với các hình thức nạp tiền/thanh toán offline (chuyển khoản qua
          I-banking, ATM), chính sách của một số Ngân hàng sẽ không hỗ trợ kiểm
          tra sau 19h00: Agribank, Đông Á, Sacombank, Vietinbank, Vietcombank.
        </p>
      </div>
